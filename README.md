...
- 仓库内**不包含运行时数据**：真实目录配置、别名表、媒体库索引、操作日志都在 `.gitignore` 里
  （它们会列出你的番剧清单与文件路径，**切勿提交**）。
- **不包含任何账号 / 密钥**：Google Programmable Search 的 Key 与 cx 由使用者在界面上填写，仅保存在**本机**配置里。
...
- 不重命名文件、不下载封面、不读取视频时长（不依赖 ffprobe）。
- 不做目录监控（手动触发扫描）。
