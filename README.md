...
> **关掉网页就会自动关闭本地服务**（默认开，免得忘记关服务）。有任务在运行时浏览器会先弹
> 「确定要离开吗」二次确认（服务端会先让正在搬的那几个文件跑完再退）；刷新页面不会误关，
> 多个标签页只关其中一个也不会关。不想要这个行为 → 网页「⚙ 设置 → ④ 关闭网页时自动关闭服务」。
...
server\bin\stop.bat              # 关闭服务（关掉网页也会自动关闭）
```
